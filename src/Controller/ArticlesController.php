...
class ArticlesController extends AbstractController
{
    ...
    public function edit(Request $request, Articles $article, ArticlesRepository $articlesRepository): Response
    {
        ...
        if ($imageFile) {
            $uploadDirectory = $this->getParameter('kernel.project_dir') . '/public/images/articles';
            $imagpath = md5(uniqid()) . '.' . $imageFile->guessExtension();
            $imageFile->move($uploadDirectory, $imagpath);
            $article->setImagpath($imagpath);
        }
